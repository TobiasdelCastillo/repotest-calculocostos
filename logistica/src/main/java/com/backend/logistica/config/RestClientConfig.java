...
public class RestClientConfig {
    ...
    // NUEVO: Cliente para GeoAPI
    @Bean("geoApiRestClient")
    RestClient geoApiRestClient(@Value("${clients.geoapi.url}") String baseUrl) {
        return RestClient.builder()
        .baseUrl(baseUrl)
        .requestInterceptor((request, body, execution) -> {
                    // 1. Buscamos quién está logueado en este momento
    ...
    // NUEVO: Cliente para el microservicio Gestión
    @Bean("gestionRestClient")
    RestClient gestionRestClient(@Value("${clients.gestion.url}") String baseUrl) {
        return RestClient.builder()
        .baseUrl(baseUrl)
        .requestInterceptor((request, body, execution) -> {
                    // 1. Buscamos quién está logueado en este momento
